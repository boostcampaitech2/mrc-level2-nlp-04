...
class RobertaQAConv_ver2(RobertaPreTrainedModel):
    def __init__(self, config):
        super(RobertaQAConv_ver2, self).__init__(config)
    ...
    def forward(self,
        ...
        query = self.query_layer(query)  # Dense Layer 를 통과 시킴. (batch_size, max_seq_length, hidden_size)
        query = torch.mean(query, 1, keepdim=True)  # Query 에 해당하는 Token Embedding 을 평균냄 (batch_size, 1, hidden_size)
        query_logits = self.query_classify_layer(query.squeeze(1))  # Query 의 종류를 예측하는 Branch (batch_size, 6)
