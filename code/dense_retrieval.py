...
class DenseRetrieval:
    ...
    def get_negative_sampling(self, dataset):
        retrieval_elastic = ElasticSearchRetrieval(data_args=self.data_args, use_negative_sampling=True)
        elasticsearch_result, _ = retrieval_elastic.retrieve(query_or_dataset=dataset, topk=self.num_neg+1)
        p_with_neg = []
        ...
        for original_target, elastic_target in zip(original_context_sets, elastic_context_sets):
            ...
            # elasticsearch로 가져온 유사한 문장에 정답이 포함될 수 있으므로 그 문장의 index를 찾는 과정 -> 나중에 해당 index를 기준으로 slicing 해줄 것이다.
            for index in range(len(elastic_target)):
