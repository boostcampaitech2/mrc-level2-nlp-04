...
class DenseRetrieval:
    def __init__(self, training_args, model_args, tokenizer, p_encoder, q_encoder, num_neg=2,
                 data_path='../data', context_path='wikipedia_documents.json'):
        self.training_args = training_args
        self.model_args = model_args
        # self.data_args = model_args
        # self.args = args
    ...
    def get_dataloader(self):
        ...
        datasets = load_from_disk(self.model_args.dataset_name)
        print(datasets)
...
def get_encoders(model_args):
    model_config = AutoConfig.from_pretrained(model_args.model_name_or_path)
    tokenizer = AutoTokenizer.from_pretrained(model_args.model_name_or_path, use_fast=True)
    if model_args.use_trained_model:
        if model_args.use_custom:
            p_encoder_path = os.path.join(model_args.output_dir, 'p_encoder')
            q_encoder_path = os.path.join(model_args.output_dir, 'q_encoder')
            p_encoder = BiLSTM_RetrievalEncoder(p_encoder_path, model_config)
            q_encoder = BiLSTM_RetrievalEncoder(q_encoder_path, model_config)
        else:
            p_encoder_path = os.path.join(model_args.output_dir, 'p_encoder')
            q_encoder_path = os.path.join(model_args.output_dir, 'q_encoder')
        ...
        if model_args.use_custom:
            p_encoder = BiLSTM_RetrievalEncoder(model_args.model_name_or_path, model_config)
            q_encoder = BiLSTM_RetrievalEncoder(model_args.model_name_or_path, model_config)
        else:
            p_encoder = RetrievalEncoder(model_args.model_name_or_path, model_config)
            q_encoder = RetrievalEncoder(model_args.model_name_or_path, model_config)
