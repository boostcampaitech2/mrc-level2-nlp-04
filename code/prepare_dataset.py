...
def make_custom_dataset(dataset_path):
    ...
    train_f = Features({'answers': Sequence(
        feature={'text': Value(dtype='string', id=None), 'answer_start': Value(dtype='int32', id=None)}, length=-1,
        id=None),
    ...
    if not os.path.isfile("/opt/ml/data/preprocess_train.pkl"):
    ...
    if 'concat' in dataset_path:
        ...
        for idx, val in enumerate(val_data):
            res = search_es(es, "wiki-index", val["question"], k)
            context_list = [(hit['_source']['document_text'], hit['_score']) for hit in res['hits']['hits']]
    ...
    if "split_wiki_400" in dataset_path:
        with open("/opt/ml/data/preprocess_wiki.json", "r", encoding = "utf-8") as f:
            wiki = json.load(f)
        new_wiki = dict()
    ...
    if "split_wiki" in dataset_path and dataset_path != "split_wiki_400":
        ...
        for i in tqdm(range(len(wiki))):
            ...
            new_wiki[str(i) + f"_1"] = {"text": data_1, "corpus_source": wiki[str(i)]["corpus_source"],
                                        "url": wiki[str(i)]["url"],
                                        "domain": wiki[str(i)]["domain"], "title": wiki[str(i)]["title"],
                                        "author": wiki[str(i)]["author"],
                                        "html": wiki[str(i)]["html"], "document_id": wiki[str(i)]["document_id"]}
            new_wiki[str(i) + f"_2"] = {"text": data_2, "corpus_source": wiki[str(i)]["corpus_source"],
                                        "url": wiki[str(i)]["url"],
                                        "domain": wiki[str(i)]["domain"], "title": wiki[str(i)]["title"],
                                        "author": wiki[str(i)]["author"], "html": wiki[str(i)]["html"],
