...
def set_datas(args):
    ...
    qa_records = [{"example_id": train_file[i]["id"], "document_title": train_file[i]["title"],
                   "question_text": train_file[i]["question"], "answer": train_file[i]["answers"]} for i in
                  range(len(train_file))]
...
if __name__ == '__main__':
    ...
    # elasticsearch-7.6.2
    parser.add_argument('--path_to_elastic', type=str, default='elasticsearch-7.15.1/bin/elasticsearch',
                        help='Path to Elastic search')
    parser.add_argument('--index_name', type=str, default='wiki-index-split-400',
                        help='Elastic search index name[wiki-index, wiki-index-split-400, wiki-index-split-800, wiki-index-split-1000]')
