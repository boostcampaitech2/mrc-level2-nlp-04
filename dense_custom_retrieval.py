...
from model.Retrieval_Encoder.retrieval_encoder import RetrievalEncoder, BertEncoder
...
class DenseRetrieval:
    ...
    def build_faiss(self, num_clusters=64) -> NoReturn:
        ...
        if os.path.isfile(indexer_path):
            ...
            tqdm(self.indexer.train(p_emb), desc="faiss index is training")
    ...
    def retrieve_bm25(
        ...
        if isinstance(query_or_dataset, str):
            doc_scores, doc_indices = get_bm25_passage(query_or_dataset, topk)
            print("[Search query]\n", query_or_dataset, "\n")
    ...
    def retrieve_faiss(
        ...
        if isinstance(query_or_dataset, str):
            ...
            with timer("get_relevant_doc_bulk_faiss"):
    ...
    def get_relevant_doc_bulk_faiss(
            self, queries: List, training_args, model_args, k: Optional[int] = 1
    ) -> Tuple[List, List]:
    ...
    def retrieve_rerank(
        ...
        if isinstance(query_or_dataset, str):
            with timer("rerank bm_25 and dpr query"):
                bm_25_scores, bm_25_indices = get_bm25_passage(query_or_dataset, 2000)
                dpr_scores, dpr_indices = self.get_relevant_doc_faiss(query_or_dataset, training_args, model_args, 2000)
            ...
            with timer("rerank bm_25 and dpr queries"):
                queries = query_or_dataset["question"]
                bm_25_scores, bm_25_indices = get_bm25_passage(queries, 2000)
                dpr_scores, dpr_indices = self.get_relevant_doc_bulk_faiss(queries, training_args, model_args, 2000)
                bm_25_scores = torch.Tensor(bm_25_scores)
                dpr_scores = torch.Tensor(dpr_scores)
                bm_25_scores = F.softmax(bm_25_scores, dim=1).numpy()
                dpr_scores = F.softmax(dpr_scores, dim=1).numpy().tolist()
                ...
                list_of_lists = bm_25_indices + dpr_indices
                max_idx = max(sum(list_of_lists, []))
                new_score_list = [[0] * (max_idx + 1) for _ in range(len(queries))]
                ...
                for i in tqdm(range(len(sorted_result)), desc="top-k selecting", total=len(sorted_result)): # q개수
                    doc_scores.append(new_score_list[i][sorted_result[i]].tolist()[:topk])
...
def get_encoders(training_args, model_args):
    ...
    if model_args.use_trained_model:
        ...
        q_encoder = BertEncoder(q_encoder_path, model_config)
        p_encoder_path = os.path.join(training_args.retrieval_output_dir, 'p_encoder')
        p_encoder = BertEncoder(p_encoder_path, model_config)
    else:
        q_encoder = BertEncoder(model_args.retrieval_model_name_or_path, model_config)
        p_encoder = BertEncoder(model_args.retrieval_model_name_or_path, model_config)
